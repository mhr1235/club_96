# Alice RAG Notes

Alice's retrieval set focuses on queer methods, visual culture, trans/body affirmation, decolonial critique, archives, coalition, and programming imagination.

Indexed chunk files:

- `chunks/alice_visual_culture_chunks.md`
- `chunks/cyber_hearts_chunks.md`
- `chunks/queer_methods_chunks.md`
- `chunks/alice_decolonial_imperial_queer_chunks.md`
- `chunks/alice_archive_trans_memory_chunks.md`
- `chunks/alice_queer_methods_mess_fieldwork_chunks.md`
- `chunks/alice_queer_theory_events_media_chunks.md`
- `chunks/alice_film_poetry_survival_language_chunks.md`

Source notes:

- `sources/alice_participant_image_notes.md`
- `sources/cyber_hearts_event.md`
- `sources/queer_methods_reading_notes.md`
- `sources/expanded_queer_methods_reading_notes.md`
- `sources/alice_jagose_safe_desire_dyke_night_notes.md`
- `sources/alice_film_poetry_survival_language_source_notes.md`
